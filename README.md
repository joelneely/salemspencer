...
* Added a pipelined parallel search (`-pipeline`/`-pp` flag) in `sspipeline.go`. The existing `-parallel` mode completes each _N_ entirely before starting _N_+1, leaving cores idle during _N_'s straggler phase. The pipelined mode fixes this by using per-_N_ `pipeJobState` structs (isolating bestWeight, result set, and WaitGroup) so _N_ and _N_+1 can run concurrently. A persistent worker pool drains a shared channel; an enqueuer goroutine submits _N_+1's sub-problems without waiting for _N_ to finish, so idle cores immediately pick up work. The result collector waits on each _N_'s WaitGroup in order before printing, preserving sequential output. Race detector confirmed no data races.
